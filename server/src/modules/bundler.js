...
const Bundler = {
  ...
  async createSplitRequests(medicine, sources, requestingHospitalId, urgency, userId) {
    const parentId = `SPLIT-${Date.now()}`;
    const requests = [];
    for (const src of sources) {
      const req = await db.createTransferRequest({
        requestingHospitalId, sourceHospitalId: src.hospital.id,
        medicine, quantityKg: src.allocatedKg, urgency: urgency || 'MEDIUM',
        reason: sources.length > 1 ? `Split fulfillment — Part of ${parentId}` : 'AI-recommended source',
        inventoryItemId: src.inventoryItemId, boxId: src.boxId,
        shelfPosition: src.shelfPosition, targetRfidUid: src.rfidUid,
        parentRequestId: parentId, isSplit: sources.length > 1, userId
      });
      requests.push(req);
    }
